...
def decoder(pred):
        ...
        for j in range(grid_num):
            for b in range(2):
                if mask[i, j, b] == 1:
                    ...
                    contain_prob = pred[i, j, b * 5 + 4] # 从pred中取出bbox的置信度
                    ...
                    if float((contain_prob * max_prob).item()) > 0.1:
    ...
    if len(boxes) == 0:
        ...
        probs = torch.cat([prob.unsqueeze(0) for prob in probs], 0)
...
def nms(bboxes, scores, threshold=0.5):
    ...
    while order.numel() > 0: # torch.numel()返回张量元素个数
        ...
        xx1 = torch.clamp(x1[i], x1[order[1:]], max=x2[order[1:]])
        yy1 = torch.clamp(y1[i], y1[order[1:]], max=y2[order[1:]])
        xx2 = torch.clamp(x2[i], min=x1[order[1:]], max=x2[order[1:]])
        yy2 = torch.clamp(y2[i], min=y1[order[1:]], max=y2[order[1:]])
...
if __name__ == "__main__":
    model = resnet50()
    print("load model...")
    model.load_state_dict(torch.load("YOLO\yolo.pth"))
    model.eval()
    image_name = "YOLO\imgs\demo.jpg"
    ...
    cv2.imwrite("YOLO\imgs\demo_result.jpg", image)